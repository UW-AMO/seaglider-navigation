...
def gen_dive(sim_params):
    ...
    depths_down = np.arange(0,sim_params.max_depth,
                            sim_params.max_depth /
                            (sim_params.n_timepoints//2))
    depths_up = (sim_params.max_depth +
                 np.arange(.1,sim_params.max_depth, sim_params.max_depth /
                                                 (sim_params.n_timepoints//2)))
    timepoints = np.linspace(start_time.value,
    ...
    if sim_params.n_timepoints % 2 == 1:
        depths = np.concatenate((depths_down.tolist(), [sim_params.max_depth],
                                 depths_up.tolist()))
    else:
...
def gen_adcp_depths(depth_df, sim_params):
    ...
    down_adcp_depths = max_adcp_depth*np.random.random_sample(
                                                (len(down_adcp_points),
                                                sim_params.adcp_bins))
    ...
    adcp_depths = np.vstack((down_adcp_depths,
                             up_adcp_depths))
...
def sim_current_profile(depths, sim_params, method='cos'):
    ...
    curr_df = pd.DataFrame(data = np.hstack([e_curr.reshape((-1,1)),
                                         n_curr.reshape((-1,1))]),
...
def select_times(depth_df, sim_params):
    ...
    if sim_params.measure_points['gps'] == 'first':
        ...
        + sim_params.measure_points['ttw']) >= 1:
        raise ValueError('Can only sample 100% of timepoints,'\
                         'check sim_params.measure_points')
    if (sim_params.measure_points['range']
        + sim_params.measure_points['ttw']) >= 1:
        raise ValueError('Can only sample 100% of timepoints,'\
                         'check sim_params.measure_points')
    unallocated_times = set(timepoints)-set(gps_times)
    num_ttw_times = int(n_timepoints * sim_params.measure_points['ttw'])
    ttw_times = pd.to_datetime(sorted(
            random.sample(unallocated_times, num_ttw_times)))
    ...
    range_times = pd.to_datetime(sorted(
            random.sample(unallocated_times, num_range_times)))
    ...
    if isinstance(sim_params.measure_points['gps'], float):
        ...
        gps_times = pd.to_datetime(sorted(
                random.sample(unallocated_times, num_gps_times)))
        unallocated_times -= set(gps_times)

    adcp_times = pd.to_datetime(sorted(list(unallocated_times)))

    return {'gps':gps_times, 'ttw':ttw_times, 'range':range_times,
            'adcp':adcp_times}

# %%
def sim_measurement_noise(depth_df, adcp_df, curr_df, v_df, ttw_times,
                          adcp_times, gps_times, range_times, sim_params):
    ...
    z_ttw_n = np.random.normal(loc=v_df.loc[ttw_times, 'ttw_n'],
                               scale=sim_params.rho_t)
    z_ttw_e = np.random.normal(loc=v_df.loc[ttw_times, 'ttw_e'],
                               scale=sim_params.rho_t)
    ...
    n_adcp_mean = (csn(adcp_df.loc[adcp_times])
                    - v_df.loc[adcp_times,'v_otg_n'].values.reshape((-1,1)))
    e_adcp_mean = (cse(adcp_df.loc[adcp_times])
                    - v_df.loc[adcp_times,'v_otg_e'].values.reshape((-1,1)))
...
def construct_load_dicts(depth_df, adcp_df, measurements, ttw_times,
    ...
    ddf = depth_df.loc[pd.Index(gps_times.union(range_times).union(
                                ttw_times).sort_values(), name='time')]
    reflect = ddf.depth > sim_params.max_depth
    ddf.loc[reflect, 'depth'] = 2*sim_params.max_depth-ddf.loc[reflect,'depth']
...
def true_solution(curr_df, v_df, final_depths, sim_params):
    ...
    cdf = curr_df.append(pd.DataFrame(index = final_depths,
                                          columns = ['curr_e', 'curr_n']))
    cdf = cdf.sort_index()
...
def simulate(sim_params):
    ...
    measurements = sim_measurement_noise(depth_df, adcp_df, curr_df, v_df,
                                         ttw_times, adcp_times, gps_times,
                                         range_times, sim_params)


    ddat, adat = construct_load_dicts(depth_df, adcp_df, measurements,
                                      ttw_times, adcp_times, gps_times,
                                      range_times, sim_params)
